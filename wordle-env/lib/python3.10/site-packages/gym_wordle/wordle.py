...
class WordleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Count feedback on this guess
        flags = self.state[self.round - 1, self.n_letters :]
        n_green = int((flags == self.right_pos).sum())
        n_yellow = int((flags == self.wrong_pos).sum())

        # Shaped reward
        reward = 0.25 * n_green + 0.10 * n_yellow - 0.05  # per-step penalty

        if correct:
            reward += 1.5  # strong solve bonus
        elif game_over:
            reward -= 1.0  # fail penalty
